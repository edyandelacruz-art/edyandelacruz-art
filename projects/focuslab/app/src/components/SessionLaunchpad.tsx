...
import { GAME_ART_URLS } from '@/lib/gameArt';
...
const GAME_CARDS: GameCard[] = [
  { name: 'Runner', title: 'Runner', description: 'Escucha mientras avanzas.', skill: 'Selective attention' },
  { name: 'Tetris', title: 'Tetris', description: 'Planifica y conserva información.', skill: 'Working memory' },
  { name: 'Road Dodge', title: 'Road Dodge', description: 'Esquiva y controla impulsos.', skill: 'Inhibitory control' },
  { name: 'Pong', title: 'Pong', description: 'Sigue el objetivo sin perder el foco.', skill: 'Sustained attention' },
  { name: 'Snake', title: 'Snake', description: 'Planifica y recupera información.', skill: 'Recall' },
  { name: 'Memory Grid', title: 'Memory Grid', description: 'Observa y repite patrones.', skill: 'Working memory' },
];

function GameArt({ game }: { game: GameName }) {
  return (
    <div
      className="lp-art lp-generated-art"
      style={{
        backgroundImage: `linear-gradient(180deg, rgba(4, 12, 26, 0.04), rgba(4, 12, 26, 0.34)), url('${GAME_ART_URLS[game]}')`,
      }}
      aria-hidden="true"
    />
  );
...
export default function SessionLaunchpad({ profile, initialGame, onBack, onLaunch }: Props) {
  ...
  return (
    <main className="launchpad-page">
      <i className="launchpad-glow one" />
      <i className="launchpad-glow two" />
      ...
      <section className="launchpad-source">
        <div className="source-tabs" role="tablist" aria-label="Fuente de contenido">
          <button
            type="button"
            className={sourceMode === 'youtube' ? 'active youtube' : ''}
            onClick={() => {
        ...
        {sourceMode === 'youtube' ? (
          <div className="youtube-source-card">
            <div className="youtube-input-shell">
              ...
              <div className="youtube-command-actions">
                ...
                <button type="button" className="primary" onClick={searchYouTube}>
                  {youtubeVideo ? 'Abrir' : 'Buscar'}
                </button>
              </div>
            </div>
            ...
            {youtubeVideo ? (
              <div className="youtube-preview-card">
                ...
                <div className="youtube-preview-meta">
                  ...
                  <div>
                    <button
                      type="button"
                      onClick={() => window.open(youtubeVideo.canonicalUrl, '_blank', 'noopener,noreferrer')}
                    >
                      Abrir en YouTube
                    </button>
                    <button
              ...
              <div className="youtube-empty-state">
                ...
                <span>
                  {youtubeInput.trim()
                    ? 'Pulsa Buscar para abrir resultados en YouTube y luego pega el enlace elegido.'
                    : 'FocusLab validará el enlace y mostrará el video antes de iniciar.'}
                </span>
              </div>
            )}
            ...
            <div className="source-material-block">
              <div className="source-material-heading">
                <div>
                  <strong>Contenido para las preguntas</strong>
                  <span>Debe corresponder al video.</span>
                </div>
                <em>OBLIGATORIO</em>
              </div>
              <input
                className="launchpad-title-input"
                value={title}
              ...
              <textarea
                ...
                rows={5}
                placeholder="Pega aquí la transcripción o un texto fiel al video. FocusLab generará las preguntas únicamente desde este contenido."
              />
              <p className="source-honesty-note">
                El enlace queda asociado a la sesión. Hasta conectar un proveedor de transcript, FocusLab no inventará ni afirmará haber extraído automáticamente el contenido del video.
              </p>
            </div>
          </div>
        ) : (
          <div className="text-source-card">
            ...
            <textarea
              ...
              onChange={event => setText(event.target.value)}
              rows={8}
              placeholder="Pega aquí el contenido que quieres estudiar. FocusLab generará preguntas trazables desde este texto."
            />
          </div>
        )}
      ...
      <section className="launchpad-games">
        <div className="launchpad-section-head">
          <div>
            <span>ELIGE TU MUNDO</span>
            <h2>¿Con qué quieres entrenar?</h2>
          </div>
        ...
        <div className="game-picker-grid">
          {GAME_CARDS.map(item => (
            <button
              type="button"
              key={item.name}
              className={`visual-game-card ${condition === 'dual_task' && game === item.name ? 'selected' : ''}`}
              onClick={() => chooseGame(item)}
            >
              <GameArt game={item.name} />
              <div className="visual-game-copy">
      ...
      <section className="launchpad-options">
          ...
          <div className="skill-chip-row">
            {SKILLS.map(item => (
              <button
                type="button"
                key={item.value}
                className={skill === item.value ? 'active' : ''}
        ...
        <div className="launchpad-option-group compact">
          ...
          <div className="condition-chip-row">
            <button
              type="button"
              className={condition === 'dual_task' ? 'active' : ''}
              onClick={() => setCondition('dual_task')}
            >
              Con juego
            </button>
            <button
              type="button"
              className={condition === 'baseline' ? 'active' : ''}
      ...
      <div className="launchpad-dock">
        ...
        <button className="launchpad-start" type="button" onClick={start}>
          {condition === 'baseline' ? 'Crear baseline' : 'Empezar'} <span>→</span>
        </button>
      </div>
    </main>
  );
