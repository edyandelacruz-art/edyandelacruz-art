...
export default function PongGame({ speed, paused, sessionKey, onStats }: Props) {
  ...
  useEffect(()=>{
    ...
    let py=105,score=0,misses=0,raf=0;
    let ball={x:380,y:140,vx:3.2,vy:2.2};
    const clamp=(v:number,a:number,b:number)=>Math.max(a,Math.min(b,v));
    const key=(e:KeyboardEvent)=>{if(pausedRef.current)return;if(e.code==='ArrowUp'){py-=26;e.preventDefault();}if(e.code==='ArrowDown'){py+=26;e.preventDefault();}py=clamp(py,18,c.height-88);};
    const pointer=(e:PointerEvent)=>{const r=c.getBoundingClientRect();py=clamp((e.clientY-r.top)*(c.height/r.height)-35,18,c.height-88);};
    ...
    const draw=()=>{raf=requestAnimationFrame(draw);const bg=ctx.createLinearGradient(0,0,0,c.height);bg.addColorStop(0,'#102a5e');bg.addColorStop(.5,'#0b1b41');bg.addColorStop(1,'#050d20');ctx.fillStyle=bg;ctx.fillRect(0,0,c.width,c.height);const halo=ctx.createRadialGradient(380,140,10,380,140,260);halo.addColorStop(0,'rgba(88,220,244,.12)');halo.addColorStop(1,'rgba(85,70,200,0)');ctx.fillStyle=halo;ctx.fillRect(0,0,c.width,c.height);ctx.save();ctx.strokeStyle='rgba(102,225,245,.18)';ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(48,32,c.width-96,c.height-64,30);ctx.stroke();ctx.restore();ctx.setLineDash([8,12]);ctx.strokeStyle='rgba(190,220,255,.16)';ctx.beginPath();ctx.moveTo(c.width/2,44);ctx.lineTo(c.width/2,c.height-44);ctx.stroke();ctx.setLineDash([]);for(let i=0;i<5;i+=1){ctx.strokeStyle='rgba(91,222,239,.055)';ctx.beginPath();ctx.arc(380,140,60+i*38,0,Math.PI*2);ctx.stroke();}
      if(!pausedRef.current){const m=.72+speedRef.current*.08;ball.x+=ball.vx*m;ball.y+=ball.vy*m;if(ball.y<42||ball.y>c.height-42)ball.vy*=-1;const aiY=clamp(ball.y-35,18,c.height-88);if(ball.x>c.width-70&&ball.x<c.width-42&&ball.y>aiY&&ball.y<aiY+70){ball.vx=-Math.abs(ball.vx);score++;onStatsRef.current(score,misses);}if(ball.x<70&&ball.x>42&&ball.y>py&&ball.y<py+70){ball.vx=Math.abs(ball.vx);score++;onStatsRef.current(score,misses);}if(ball.x<0){misses++;onStatsRef.current(score,misses);reset(1);}if(ball.x>c.width){reset(-1);}}
      rounded(52,py,13,72,7,'#66eff2','#66eff2');const ai=clamp(ball.y-35,18,c.height-88);rounded(c.width-65,ai,13,72,7,'#a978ff','#a978ff');ctx.save();ctx.shadowColor='#fff';ctx.shadowBlur=22;const orb=ctx.createRadialGradient(ball.x-3,ball.y-4,1,ball.x,ball.y,10);orb.addColorStop(0,'#ffffff');orb.addColorStop(.55,'#9cf8ff');orb.addColorStop(1,'#5b7cff');ctx.fillStyle=orb;ctx.beginPath();ctx.arc(ball.x,ball.y,9,0,Math.PI*2);ctx.fill();ctx.restore();ctx.fillStyle='rgba(230,241,255,.8)';ctx.font='600 13px system-ui';ctx.fillText('SIGUE LA ESFERA · ↑ ↓ / PUNTERO',20,27);ctx.fillStyle='#7cecf3';ctx.font='800 12px system-ui';ctx.fillText(`RALLY ${score}`,20,48);ctx.fillStyle='#a98fff';ctx.fillText(`ERRORES ${misses}`,650,48);};
