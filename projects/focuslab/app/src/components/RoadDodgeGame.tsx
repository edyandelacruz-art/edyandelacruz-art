...
export default function RoadDodgeGame({ speed, paused, sessionKey, onStats }: Props) {
  ...
  useEffect(() => {
    const c = canvasRef.current; if (!c) return; const prepared = prepareHiDpiCanvas(c); if (!prepared) return; const { ctx } = prepared; const art = createGameArt('Road Dodge');
    ...
    const draw = () => {
      ...
      const bg = ctx.createLinearGradient(0,0,0,GAME_HEIGHT); bg.addColorStop(0,'#172c5c'); bg.addColorStop(.38,'#0b1d43'); bg.addColorStop(1,'#060f24'); ctx.fillStyle = bg; ctx.fillRect(0,0,GAME_WIDTH,GAME_HEIGHT);
      ...
      ctx.fillStyle='#0a1f3d';ctx.fillRect(0,70,GAME_WIDTH,210);
      ...
      drawGameArt(ctx, art, GAME_WIDTH, GAME_HEIGHT, 1, 0.54);

      if(!pausedRef.current){frame+=1;obstacles.forEach(o=>o.y+=2.2+speedRef.current*.5);obstacles.forEach(o=>{if(o.y>GAME_HEIGHT+60){o.y=-100-Math.random()*180;o.lane=Math.floor(Math.random()*3);statsRef.current.score++;onStatsRef.current(statsRef.current.score,statsRef.current.collisions);}});const py=205;obstacles.forEach(o=>{if(o.lane===lane&&o.y+48>py&&o.y<py+58&&frame-lastHit>48){statsRef.current.collisions++;lastHit=frame;onStatsRef.current(statsRef.current.score,statsRef.current.collisions);}});}
