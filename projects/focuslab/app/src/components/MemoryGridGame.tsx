...
export default function MemoryGridGame({ speed, paused, sessionKey, onStats }: Props) {
  ...
  useEffect(()=>{pausedRef.current=paused;},[paused]); useEffect(()=>{onStatsRef.current=onStats;},[onStats]);

  useEffect(()=>{
    const c=canvasRef.current;if(!c)return;const prepared=prepareHiDpiCanvas(c);if(!prepared)return;const {ctx}=prepared;const art=createGameArt('Memory Grid');
    const n=4,size=46,gap=12,boardW=n*size+(n-1)*gap,ox=GAME_WIDTH/2-boardW/2,oy=48;
    ...
    const click=(e:PointerEvent)=>{if(pausedRef.current||show)return;const r=c.getBoundingClientRect();const x=(e.clientX-r.left)*(GAME_WIDTH/r.width),y=(e.clientY-r.top)*(GAME_HEIGHT/r.height);const col=Math.floor((x-ox)/(size+gap)),row=Math.floor((y-oy)/(size+gap));if(col<0||col>=n||row<0||row>=n)return;const id=row*n+col;if(chosen.has(id))return;chosen.add(id);if(targets.has(id)){score++;onStatsRef.current(score,errors);}else{errors++;onStatsRef.current(score,errors);}if(chosen.size>=4)setTimeout(newRound,350);};
