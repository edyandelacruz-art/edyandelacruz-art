...
export default function RunnerGame({ speed, paused, sessionKey, onStats }: RunnerGameProps) {
  ...
  useEffect(() => {
    ...
    let obstacle = { x: canvas.width + 120, y: ground - 48, w: 34, h: 48 };
    let lastHit = -1000;
    ...
    const draw = () => {
      ...
      sky.addColorStop(0, '#132d63');
      sky.addColorStop(.52, '#0d1d44');
      ...
      moon.addColorStop(.28, '#79dff7');
      moon.addColorStop(1, 'rgba(87,118,221,0)');
      ctx.fillStyle = moon;
      ctx.beginPath();
      ctx.arc(620, 48, 62, 0, Math.PI * 2);
      ctx.fill();
      ...
      for (let i = -1; i < 4; i += 1) {
        ...
        ctx.beginPath();
        ctx.moveTo(x, ground);
        ctx.quadraticCurveTo(x + 110, 120, x + 250, ground);
        ctx.closePath();
        ctx.fill();
      }
      ...
      for (let i = -1; i < 5; i += 1) {
        ...
        ctx.beginPath();
        ctx.moveTo(x, ground);
        ctx.quadraticCurveTo(x + 80, 155, x + 190, ground);
        ctx.closePath();
        ctx.fill();
      }

      ctx.fillStyle = '#081833';
      ctx.fillRect(0, ground, canvas.width, canvas.height - ground);
      const floorGlow = ctx.createLinearGradient(0, ground - 6, 0, ground + 26);
      floorGlow.addColorStop(0, 'rgba(95,240,247,.85)');
      floorGlow.addColorStop(.15, 'rgba(95,240,247,.12)');
      floorGlow.addColorStop(1, 'rgba(95,240,247,0)');
      ctx.fillStyle = floorGlow;
      ctx.fillRect(0, ground - 6, canvas.width, 32);
      ctx.fillStyle = '#71eff5';
      ctx.fillRect(0, ground - 2, canvas.width, 3);

      for (let x = -((frame * speedRef.current * 2) % 78); x < canvas.width; x += 78) {
        ctx.fillStyle = 'rgba(95,136,210,.12)';
        ctx.fillRect(x, ground + 22, 46, 3);
      }
      ...
      if (!pausedRef.current) {
        ...
        if (obstacle.x < -obstacle.w) {
          obstacle = { ...obstacle, x: canvas.width + 100 + Math.random() * 180 };
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath();
      ctx.ellipse(player.x + 23, ground + 3, 31, 7, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();

      ctx.save();
      ctx.shadowColor = '#5debf1';
      ctx.shadowBlur = 22;
      const suit = ctx.createLinearGradient(player.x, player.y, player.x + player.w, player.y + player.h);
      suit.addColorStop(0, '#75f5ef');
      suit.addColorStop(.55, '#5888ff');
      suit.addColorStop(1, '#8a5bf5');
      ctx.fillStyle = suit;
      rounded(player.x + 9, player.y + 20, 26, 34, 12);
      ctx.fillStyle = '#dffcff';
      ctx.beginPath();
      ctx.arc(player.x + 22, player.y + 13, 12, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#183b75';
      ctx.beginPath();
      ctx.arc(player.x + 22, player.y + 14, 7, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = '#74eef5';
      ctx.lineWidth = 5;
      ctx.lineCap = 'round';
      ctx.beginPath();
      ctx.moveTo(player.x + 14, player.y + 49);
      ctx.lineTo(player.x + 6, player.y + 60);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(player.x + 30, player.y + 49);
      ctx.lineTo(player.x + 39, player.y + 59);
      ctx.stroke();
      ctx.restore();

      ctx.save();
      ctx.shadowColor = '#ff7f91';
      ctx.shadowBlur = 18;
      const block = ctx.createLinearGradient(obstacle.x, obstacle.y, obstacle.x + obstacle.w, obstacle.y + obstacle.h);
      block.addColorStop(0, '#ffbd72');
      block.addColorStop(.45, '#ff767f');
      block.addColorStop(1, '#8f426e');
      ctx.fillStyle = block;
      rounded(obstacle.x, obstacle.y, obstacle.w, obstacle.h, 8);
      ctx.fillStyle = 'rgba(255,255,255,.3)';
      rounded(obstacle.x + 5, obstacle.y + 5, obstacle.w - 10, 6, 3);
      ctx.restore();

      ctx.fillStyle = 'rgba(224,238,255,.78)';
      ctx.font = '600 13px system-ui';
      ctx.fillText('SALTA · ESPACIO / ↑ / TOQUE', 20, 27);
      ctx.fillStyle = 'rgba(104,241,244,.9)';
      ctx.font = '800 12px system-ui';
      ctx.fillText(`RACHA ${statsRef.current.score}`, 20, 48);
    };
